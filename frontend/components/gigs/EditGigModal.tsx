...
import { Pencil, Trash2, IndianRupee, Loader2, AlertTriangle } from "lucide-react";
...
export function EditGigModal({ gig, onUpdate }: EditGigModalProps) {
  ...
  const handleDelete = async () => {
    if (!confirm("Are you sure? All received bids will be permanently deleted.")) return;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="cursor-pointer h-9 border-slate-200 text-slate-600 hover:bg-slate-50 font-semibold gap-2">
      ...
      <DialogContent className="sm:max-w-[480px] p-0 overflow-hidden border-none shadow-2xl">
        <DialogHeader className="p-6 bg-slate-50/80 border-b border-slate-100">
          <DialogTitle className="text-2xl font-bold text-brand">Modify Gig</DialogTitle>
        ...
        <form onSubmit={handleUpdate} className="p-6 space-y-5 pt-0">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="description" className="text-sm font-bold text-slate-700">Detailed Description</Label>
            <Textarea
              id="description"
              rows={5}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="budget" className="text-sm font-bold text-slate-700">Budget Amount</Label>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 mt-2 border-t border-slate-100">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Update Gig"}
              </Button>
            </div>
          </div>
        </form>
