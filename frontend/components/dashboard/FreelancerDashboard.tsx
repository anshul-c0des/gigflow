...
export default function FreelancerDashboard() {
  ...
  useEffect(() => {   // fetch my bids on mount
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="border-none shadow-sm bg-white">
          ...
          <CardContent>
            ...
            <p className="text-xs text-freelancer font-medium mt-1">
              Win rate:{" "}
              {bidsSent > 0 ? Math.round((gigsWon / bidsSent) * 100) : 0}%
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-sm bg-freelancer text-white relative overflow-hidden group">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 relative z-10">
            <CardTitle className="text-sm font-semibold uppercase tracking-wider opacity-90">
              Opportunities
            </CardTitle>
          </CardHeader>
          <CardContent className="relative z-10">
            ...
            <Button
              ...
              size="sm"
              variant="secondary"
              className="w-full bg-white text-teal-600 hover:bg-slate-50 border-none shadow-md"
            >
              <Link
                href="/gigs"
          ...
          <CardContent className="p-0">
            {bids.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <Link
                  href="/gigs"
                  className="text-teal-600 text-sm font-semibold hover:underline mt-1"
                >
              ...
              <div className="divide-y divide-slate-50">
                {bids.map((bid) => (
                  <div
                    key={bid._id}
                    className="group flex flex-col md:flex-row md:items-center justify-between p-6 hover:bg-teal-50 transition-all"
                  >
                    <div className="flex gap-4">
                      <div
                        className={`hidden sm:flex h-12 w-12 items-center justify-center rounded-xl border transition-colors ${
                          bid.status === "hired"
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-4 mt-4 md:mt-0">
                      ...
                      <Button
                        ...
                        size="sm"
                        asChild
                        className="text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg px-4"
                      >
