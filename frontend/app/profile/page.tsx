...
export default function ProfilePage() {
  ...
  useEffect(() => {   // fetch my bids and gigs
    const fetchStats = async () => {
      try {
        ...
        const myBids = Array.isArray(bidRes.data)
          ? bidRes.data
          : bidRes.data.bids || [];
        const myGigs = Array.isArray(gigRes.data)
          ? gigRes.data
          : gigRes.data.gigs || [];
        ...
        setStats({  
          ...
          totalEarned: myBids
            .filter((b: any) => b.status === "hired")
            .reduce((s: number, b: any) => s + b.amount, 0),
          gigsCreated: myGigs.length,
          totalBidsReceived: myGigs.reduce(
            (s: number, g: any) => s + (g.bids?.length || 0),
            0
          ),
          totalSpent: myGigs
            .filter((g: any) => g.status === "assigned")
            .reduce((s: number, g: any) => s + (Number(g.budget) || 0), 0),
        });
      } catch (err) {
  ...
  if (loading)
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        ...
        <div className="animate-pulse text-emerald-600 font-semibold tracking-wide">
          Initializing GigFlow Profile...
        </div>
      </div>
    );
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 px-4">
        <Card className="max-w-md w-full p-10 text-center shadow-xl rounded-[2rem] border-slate-100 bg-white">
          ...
          <Button className="w-full bg-emerald-950 hover:bg-emerald-900 rounded-xl h-12">
            Return to Login
          </Button>
        </Card>
      </div>
          ...
          <div className="flex-1 space-y-3 z-10">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-6 text-sm font-semibold text-slate-500">
              <span className="flex items-center gap-2">
                <Mail className="h-4 w-4 text-emerald-600" /> {user.email}
              </span>
              <span className="flex items-center gap-2">
                <Calendar className="h-4 w-4 text-emerald-600" /> Joined Jan
                2026
              </span>
            </div>
          </div>
              ...
              <Card className="border-none bg-freelancer text-white shadow-lg shadow-teal-500/20 rounded-3xl">
                <CardContent className="px-8 py-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-teal-50/80 font-bold text-xs uppercase tracking-widest mb-2">
                        Income
                      </p>
            ...
            <div className="grid gap-5">
              ...
              <Card className="border-none bg-client text-white shadow-lg shadow-emerald-950/20 rounded-3xl">
                <CardContent className="px-8 py-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-emerald-400/80 font-bold text-xs uppercase tracking-widest mb-2">
                        {" "}
                        Outflow
                      </p>
                      <p className="text-4xl font-black">
    ...
    <Card className="border-slate-100 rounded-2xl bg-white shadow-sm">
      <CardContent className="px-6 py-3 flex items-center justify-between">
        <div className="space-y-1">
          <p className="text-[10px] uppercase font-black text-slate-400 tracking-[0.15em]">
            {title}
          </p>
