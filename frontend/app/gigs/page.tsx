...
useEffect(() => {
  ...
  return (
        ...
        <header className="space-y-6">
          <div className="space-y-2 pb-2">
            <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">
              Find your next <span className="text-brand underline decoration-teal-500/20 underline-offset-8">gig</span>
            </h1>
          </div>
    ...
    <div className="flex h-screen items-center justify-center bg-slate-50">
         <Zap className="h-8 w-8 animate-pulse text-brand" /> Searching gigs…
  </div>
) : gigs.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
              <p className="text-slate-500 font-medium">No gigs found. Check back later!</p>
            </div>
          ) : (
            gigs.map((gig) => (
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between px-6 md:px-8 py-3 gap-6">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-slate-500">
                          <div className="flex items-center gap-1.5">
                            <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-[10px] text-slate-600">
                             ...
                             Posted by <span className="text-brand">{gig.owner.name}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 pt-6 md:pt-0 border-t md:border-t-0 md:border-l border-slate-100 md:pl-8">
                        <div className="text-right">
                          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Budget</p>
                          <p className="text-2xl font-black text-slate-900">₹{gig.budget?.toLocaleString() || "Negotiable"}</p>
                        </div>
...
function Button({ className, variant, children, ...props }: any) {
  ...
  return (
    <button className={`inline-flex items-center justify-center rounded-md font-medium transition-colors ${variants[variant as keyof typeof variants]} ${className}`} {...props}>
      {children}
    </button>
  );
}