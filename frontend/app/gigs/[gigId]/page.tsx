...
export default function GigDetailPage() {
  ...
  const { gigId } = useParams();   // exract gigId from params
  ...
  const isSpecificOwner = user?.id === gig?.owner._id;   // is user owner of this gig
  const canPlaceBid = !isSpecificOwner && !loading && gig?.status === "open";   // can user place a bid
  const hasAlreadyBid = bids.some((b) => b.freelancer._id === user?.id);   // did user already bid
  ...
  useEffect(() => {   // socket init
    ...
    return () => {
      socket.off("bid:created", handleNewBid);
    };
  }, [user, isSpecificOwner]);

  useEffect(() => {   // load gig details
  ...
  const handleHire = async (bidId: string) => {   // hire freelancer
    try {
      ...
      <div className=" mb-8">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {isSpecificOwner && gig.status === "open" && (
              <EditGigModal gig={gig} onUpdate={setGig} />
            )}
          </div>
        </div>
      </div>
          ...
          <section className="space-y-3">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-slate-500 font-medium">
              <span className="flex items-center gap-1.5">
                <User className="w-4 h-4" /> {gig.owner.name}
              </span>
              <span className="flex items-center gap-1.5">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="text-slate-500">
                  {bids.length}
                  {bids.length === 1 ? " Applicant" : " Applicants"}{" "}
                  {bids.length === 0 ? " yet" : ""}
                </Badge>
              </div>
                      ...
                      <div className="flex flex-col items-end gap-3 min-w-[120px]">
                        <Badge
                          className={
                            bid.status === "hired"
                              ? "bg-emerald-500"
                              : "bg-slate-100 text-slate-600 shadow-none border-none"
                          }
                        >
              ...
              {hasAlreadyBid ? (
                <div className="bg-white/10 p-4 rounded-xl text-center space-y-2 border border-white/20">
                  ...
                  <Badge className="bg-teal-500 text-white border-none">
                    {bids
                      .find((b) => b.freelancer._id === user?.id)
                      ?.status.toUpperCase()}
                  </Badge>
                </div>
              ) : canPlaceBid ? (
                ...
                <p className="text-center text-emerald-400 text-sm italic">
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-lg bg-emerald-900 text-white overflow-hidden sticky top-18">
            <CardContent className="p-8 py-3 space-y-6">
              <p className="text-white text-center text-2xl font-bold uppercase tracking-widest mb-5 -mt-2">
                Summary Card
              </p>
              <div>
